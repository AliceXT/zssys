...
class WeixinModel extends Model {
	...
	/* ========================发送被动响应消息 begin================================== */
	/* 回复文本消息 */
	public function replyText($content, $toUserName='') {
		$msg ['Content'] = $content;
		$this->_replyData ( $msg, 'text' ,$toUserName);
	}
	/* 回复图片消息 */
	...
	/* 发送回复消息到微信平台 */
	private function _replyData($msg, $msgType,$toUserName = '') {
		if($toUserName)
		{
	...
	/*上传临时素材*/
	public function uploadTmpFile($file, $type = 'image', $acctoken = '')
	{
		$post_data ['type'] = $type;
		...
		$url = "https://api.weixin.qq.com/cgi-bin/media/upload?access_token=$acctoken&type=$type";
		$ch = curl_init ();
		curl_setopt ( $ch, CURLOPT_POST, 1 );
		curl_setopt ( $ch, CURLOPT_URL, $url );
		curl_setopt ( $ch, CURLOPT_POSTFIELDS, $post_data );
