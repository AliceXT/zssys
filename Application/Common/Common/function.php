 ...
 * @author huajie <banhuajie@163.com>
 */
function action_log($action = null, $model = null, $record_id = null, $user_id = null,$field_array=null) {
	// 参数检查
	...
	if (! empty ( $action_info ['log'] )) {
		if (preg_match_all ( '/\[(\S+?)\]/', $action_info ['log'], $match )) {
			...
			foreach ( $match [1] as $value ) {
				$param = explode ( '|', $value );
				if (isset ( $param [1] )) {
					if($param[1] == 'get_action_field'){
						$key = $param[0];
						$replace [] = $field_array[$key];
					}else{
...
function get_jsapi_ticket($token = '') {
	...
	if (@array_key_exists ( 'ticket', $tempArr )) {
		S ( $key, array('ticket'=>$tempArr ['ticket'],'timestamp'=>time()), $tempArr['expires_in'] );
		return S($key);
	} else {
		return 0;
	}
...
function safe($text, $type = 'html') {
	...
	$all_tags = $form_tags . $html_tags . '<!DOCTYPE><html><head><title><body><base><basefont><script><noscript><applet><object><param><style><frame><frameset><noframes>';
	// 过滤标签
 ...
 * aciton 增加的函数
 * get_wx_nickname
 * 根据shop_user表中的id获得openid,继而得到用户昵称
 */
function get_wx_nickname($id){
	if(empty($id)){
		return '';
	}
	$map['id'] = $id;
...
*	裁剪长文，使其正确的省略
**/
function cutstr($string, $length = 14,$charset = "utf-8",$dot = "...") {//字符，截取长度，字符集，结尾符 
...
*	根据订单ID链接订单详情
**/
function add_order_link($id){
	$param['id'] = $id;
	$url = addons_url("Shop://Order/more",$param);
	$str = '<a href="'.$url.'">'.$id.'</a>';
	return $str;
}
/**
...
*	根据openid链接用户详情
**/
function add_user_link($openid){
	$param['id'] = $openid;
	$url = addons_url("Shop://User/more",$param);
	$str = '<a href="'.$url.'">'.$openid.'</a>';
	return $str;
}
/**
*	@author AliceXT
*	根据fee费用的int值，将其设置为number_format金额格式
**/
function change_fee($fee){
	return number_format($fee / 100 , 2);
}

/**
*	@author AliceXT
*	清除当前页面的openid,重新获取openid
**/
function reget_openid(){
	$token = get_token();
	// 移除url上的openid
	$param = I('get.');
	unset($param['openid']);
	$param['reset'] = 1;// 标识已经刷新
...
/**
*	@author AliceXT
*	在openid的后面记录该openid的昵称
**/
function openidLinkNickname($openids){
	$openidArr = explode("\r\n", $openids);
...
*	检查合伙人身份及列表查重
**/
function checkPartner($openids){
	$openidArr = explode("\r\n", $openids);
	$openidArr = array_unique($openidArr); 
	if(empty($openidArr[0]))
		return true;
	foreach($openidArr as $str){
		if(empty($str)){
			continue;
...
*	获得多规格ID
**/
function get_multi($multi = NULL){
	if ($multi !== NULL) {
		session ( 'multi', $multi );
	} elseif (! empty ( $_REQUEST ['multi'] )) {
		session ( 'multi', $_REQUEST ['multi'] );
	}
	$multi = session ( 'multi' );
	if (empty ( $multi )) {
		return - 1;
...
// 根据ID获得多规格名字
function get_multi_name($multi_ids){
	// $ids = explode(",", $multi_ids);
	$map['id'] = array("in",$multi_ids);
	$map['token'] = get_token();
...
function change_multis($multis){
	$arr = json_decode($multis,true);
	$map['token'] = get_token();
	foreach ($arr as $key => $value) {
		$map['id'] = $key;
		$multi = M('shop_multi')->where($map)->find();
		$map['id'] = $value;
		$option = M('shop_option')->where($map)->find();
		$str = empty($str) ? $multi['show_name'] ."：".$option['name'] : $str .",".$multi['show_name'] ."：".$option['name'];
	}
	return $str;
}
// 生成签名字符串
