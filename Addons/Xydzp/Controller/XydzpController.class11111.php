    ...
    function join()
    {
		...
		if($this->_is_starttime($xydzp_id)){
			if($this->_is_overtime($xydzp_id)){
				...
				$fix      = C("DB_PREFIX");
				//查询奖品列表		
				$jplist   = M('xydzp_jplist')->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_jplist.xydzp_option_id=' . $fix . 'xydzp_option.id')->field($fix . "xydzp_option.title,gailv,xydzp_id,xydzp_option_id,isdf," . $fix . "xydzp_option.num as kcnum," . $fix . "xydzp_option.miaoshu")->where("xydzp_id=$xydzp_id")->order('type asc,gailv asc')->select();
				//按奖品等级计算概率
    ...
    private function getResult($jplist, $xydzp_id, $openid, $token)
    {
        ...
        if ($row != 0) {
            ...
            foreach ($jplist as $key => $val) {
				...
				$m="id = ".$val['xydzp_option_id'];
				$dd = M('xydzp_option')->where( $m )->find();
				//$test = M('xydzp_option')->getlastsql();
				if($dd['num']<1){
					$arr[] = 0;	
				}else{
               		$arr[] = $val['gailv'];
