...
class DealController extends WidgetController {
	...
	function show($widget) {
		...
		$product = $Product->where ( $map )->find ();
		$product['discount_price'] = number_format($product['discount_price']/100, 2);
		$product['market_price'] = number_format($product['market_price']/100, 2);
		$product['express_fee'] = number_format($product['express_fee']/100, 2);
		$product['multis'] = $Product->getMultiWithOption($product['multi_ids']);
